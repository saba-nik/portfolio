(function() {
      ...
      const translations = {
        fa: {
          ...
          projects_sub: 'چند تا از کارهام؛ چیزهای دیگه‌ای هم در راهه، فعلاً قفلن 👀',
          ...
          skills: ['TypeScript', 'React', 'Next.js', 'Node.js', 'PostgreSQL', 'Ren\'Py', 'Godot', 'C#', 'Three.js',
            'Tailwind', 'Prisma', 'Python', 'نقاشی دیجیتال', 'مدل‌سازی سه‌بعدی', 'رندرینگ', 'هوش مصنوعی', 'شبکه', 'توسعه اپلیکیشن'
          ],
          ...
          projects: [
            ...
            },
            {
              ...
              bannerPosition: 'top',
              short: 'یه پروژه‌ی جدید داره ساخته می‌شه… فعلاً همه‌چیزش رازه.',
              tags: ['؟؟؟', '؟؟؟', '؟؟؟'],
              lockedLabel: 'قفل شده',
              lockedHint: 'این پست هنوز باز نشده. بعداً سر بزن!'
            },
            {
              title: '؟؟؟',
              locked: true,
              status: 'به‌زودی',
              tagAvatar: '؟',
              banner: 'https://raw.githubusercontent.com/saba-nik/portfolio/main/banner.jpg',
              bannerPosition: 'center',
              short: 'یه پروژه‌ی مرموز دیگه هم توی راهه…',
              tags: ['؟؟؟', '؟؟؟'],
              lockedLabel: 'قفل شده',
              lockedHint: 'این پست هنوز باز نشده. بعداً سر بزن!'
            }
        ...
        en: {
          ...
          projects_sub: "A few things I've shipped — a couple more are on the way, still locked 👀",
          ...
          skills: ['TypeScript', 'React', 'Next.js', 'Node.js', 'PostgreSQL', 'Ren\'Py', 'Godot', 'C#', 'Three.js',
            'Tailwind', 'Prisma', 'Python', 'Digital Painting', '3D Modeling', 'Rendering', 'AI', 'Networking', 'App Development'
          ],
          ...
          projects: [
            ...
            },
            {
              ...
              banner: 'https://raw.githubusercontent.com/kochinatsu/Studio/main/baner.jpg',
              bannerPosition: 'top',
              short: "Something new is being built… still a secret for now.",
              tags: ['???', '???', '???'],
              lockedLabel: 'Locked',
              lockedHint: "This post isn't unlocked yet. Check back soon!"
            },
            {
              title: '???',
              locked: true,
              ...
              banner: 'https://raw.githubusercontent.com/saba-nik/portfolio/main/banner.jpg',
              bannerPosition: 'center',
              short: "Another mysterious project is on the way…",
              tags: ['???', '???'],
              lockedLabel: 'Locked',
              lockedHint: "This post isn't unlocked yet. Check back soon!"
            }
      ...
      function buildPosts(t) {
        ...
        list.innerHTML = t.projects.map((p, i) => {
          ...
          const gameFloats = ['❀','⋆','࿔','✮','♪','ᶻ','𐰁','✦','𖤐','★','✴︎','♡','✧','｡','˖','ֶ','ᵔ','ᯓ','݁','ˊ','ˎ','˗','✧','♡','✧','·']
            .map(ch => `<span class="game-float">${ch}</span>`).join('');

          if (p.locked) {
            return `
            <article class="post-card locked-card card-glow">
              <div class="post-head">
              ...
              <div class="post-banner locked-banner" data-locked="1" tabindex="0" role="button" aria-label="${p.lockedLabel}" style="${bannerStyle}">
                ${gameFloats}
                <span class="status-pill">${p.status}</span>
                <span class="banner-title">${p.title}</span>
                <div class="lock-overlay">
                  <span class="lock-icon">${icon('lock')}</span>
                  <span class="lock-label">${p.lockedLabel}</span>
                </div>
                <div class="sweep"></div>
              </div>
              <div class="post-body">
                <p class="post-desc blur-locked">${p.short}</p>
                <div class="post-tags blur-locked">${p.tags.map(x => `<span>${x}</span>`).join('')}</div>
                <div class="post-actions">
                  <button type="button" class="primary locked-btn" data-locked="1" aria-disabled="true">${icon('lock')}<span>${p.lockedLabel}</span></button>
                </div>
              </div>
            </article>`;
          }
        ...
        document.querySelectorAll('.open-project, .post-banner:not(.locked-banner)').forEach(el => {
        ...
        document.querySelectorAll('[data-locked="1"]').forEach(el => {
          ...
          const hint = t2.projects.find(p => p.locked) ? t2.projects.find(p => p.locked).lockedHint : '';
          el.addEventListener('click', () => showToast(hint));
          el.addEventListener('keydown', e => { if (e.key === 'Enter') showToast(hint); });
        });
